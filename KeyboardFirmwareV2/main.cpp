...
#define BLE_HIDS_DEF(_name,                                              \
    ...
    {                                                                    \
		.evt_handler = ble_hids_evt_handler_t (),\
		.error_handler = ble_srv_error_handler_t (),\
		.service_handle = 0,\
		.protocol_mode_handles = ble_gatts_char_handles_t (),\
		.inp_rep_count = 0,\
		.inp_rep_array = {0},\
		.outp_rep_count = 0,\
		.outp_rep_array = {0},\
		.feature_rep_count = 0,\
		.feature_rep_array = {0},\
		.rep_map_handles = ble_gatts_char_handles_t (),\
		.rep_map_ext_rep_ref_handle = 0,\
		.boot_kb_inp_rep_handles = ble_gatts_char_handles_t (),\
		.boot_kb_outp_rep_handles = ble_gatts_char_handles_t (),\
		.boot_mouse_inp_rep_handles = ble_gatts_char_handles_t (),\
		.hid_information_handles = ble_gatts_char_handles_t (),\
		.hid_control_point_handles = ble_gatts_char_handles_t (),\
        .p_link_ctx_storage = &CONCAT_2(_name, _link_ctx_storage),          \
		.p_inp_rep_init_array = NULL, \
		.p_outp_rep_init_array = NULL, \
		.p_feature_rep_init_array = NULL, \
    };                                                                   \
...
static void dis_init(void)
{
    ...
    ble_srv_ascii_to_utf8(&dis_init_obj.manufact_name_str, const_cast<char*>(MANUFACTURER_NAME));
